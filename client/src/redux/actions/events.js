...
import { DAY, DAYS_IN_WEEK } from "../../constants/constants";
import { serverRequest } from "../../helpers/server";
...
export function addEvent(data, dispatch) {
  ...
  if (data.week && data.week.length !== DAYS_IN_WEEK)
    data.week = data.week.concat(
      new Array(DAYS_IN_WEEK - data.week.length).fill(null)
    );
  ...
  return serverRequest(data, "/event", "POST", token).then(
    ([response, json]) => {
      if (response.status === 403)
        throw new SubmissionError({ _error: "Adding failed" });
      if (json.errors) {
...
export function updateEvent(event, dispatch) {
  ...
  if (event.week && event.week.length !== DAYS_IN_WEEK)
    event.week = event.week.concat(
      new Array(DAYS_IN_WEEK - event.week.length).fill(null)
    );
  if (event.group && typeof event.group === "object")
    event.group = event.group._id;
  ...
  if (dispatch) {
    const token = localStorage.getItem("token");
    return serverRequest(event, `/event/${event._id}`, "PUT", token).then(
      ([response, json]) => {
        if (response.status === 403)
          throw new SubmissionError({ _error: "Updating failed" });
        if (json.errors) {
          const jsonErr = json.errors;
          const errors = {};
          for (let prop in json.errors) {
